...
const KINDS = Object.keys(CONSTRAINT_KINDS) as ConstraintKind[];
...
export function Constraints({
	...
	const canAdd = selected.length >= 2 && available.length > 0;
		...
		<div className={styles.constraints} data-role="constraints">
			<div className={styles.head}>
				<span className={styles.hint}>
					Rules the design must obey. Select two or more layers to add one.
				</span>
				<select
					...
					title={
						canAdd
							...
							: "Select two or more layers that share a property"
					}
					onChange={(e) => {
				...
				return (
					...
					>
						<div className={styles.ruleHead}>
							...
							{CONSTRAINT_KINDS[c.kind].counted ? (
								<input
									type="number"
