...
import { METRE_IN_EMU, type Triangles, gltfWriter, parseGltfFile } from "./gltf.ts";
...
test("a glTF becomes document nodes in the layer graph", async () => {
	const imported = await importGltf(sourceFile(), { source: "chair.gltf", id: counter() });
...
test("a material arrives as ordinary props, not as a blob's private business", async () => {
	const imported = await importGltf(sourceFile(), { id: counter() });
...
test("the payload is per node, holds geometry alone, and is addressed by its hash", async () => {
	const imported = await importGltf(sourceFile(), { id: counter() });
	assert.equal(imported.assets.length, 1);
	const asset = imported.assets[0];
	assert.ok(asset);
	// A hex SHA-256, which is what an `AssetStore` keys a payload by.
	assert.match(asset.id, /^[0-9a-f]{64}$/);
	const model = imported.nodes[0]?.children?.[0];
	assert.equal(model?.mesh?.asset, asset.id);
	assert.equal(model?.mesh?.source, undefined);
	assert.equal(model?.mesh?.triangles, 2);
	assert.equal(imported.triangles, 2);
	assert.equal(asset.info.triangles, 2);

	const payload = parseGltfFile(asset.payload);
	// No material in the payload: the file's material became props on the node,
	// which is where a designer can change it, and a second copy would be a
	// second answer to what colour the chair is.
	assert.equal(payload.json.materials, undefined);
	assert.equal(payload.json.meshes?.length, 1);
	// Centred on its own origin, because a `model`'s box is where its picture is
	// and a rotation is about a centre.
	const bounds = payload.json.accessors?.[0];
	assert.deepEqual(bounds?.min, [-2, -1, -0.5]);
	assert.deepEqual(bounds?.max, [2, 1, 0.5]);
});

test("the node's stated bounds are the box the vertices occupy", async () => {
	const imported = await importGltf(sourceFile(), { id: counter() });
	const model = imported.nodes[0]?.children?.[0];
...
test("a file's name follows it, and a source is kept for a relink", async () => {
	const imported = await importGltf(sourceFile(), {
		source: "chair.glb",
		name: "The chair",
		id: counter(),
	});
	assert.equal(imported.nodes[0]?.name, "The chair");
	assert.equal(imported.nodes[0]?.children?.[0]?.mesh?.source, "chair.glb");
});
...
test("what a document cannot hold is named rather than dropped in silence", async () => {
...
test("a buffer in another file is reported, not silently empty", async () => {
	const imported = await importGltf(
...
test("a stretched parent over a rotated child is named as the shear it is", async () => {
	const writer = gltfWriter();
...
test("a mesh drawn in two materials becomes two nodes, and says so", async () => {
...
test("the imported node stands exactly where the file put the geometry", async () => {
	const imported = await importGltf(sourceFile(), { id: counter() });
	const root = imported.nodes[0];
	assert.ok(root);
...
test("a rotation survives the crossing in both directions", async () => {
